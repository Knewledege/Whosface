...
public struct Classifier {
    ...
    public func classify(_ x_image: Tensor) -> ([Float],Int) {
        ...
        let h_pool2_flat = h_pool2.reshaped([1, (Dimension(7) * Dimension(7) * 64)])
        let h_fc1 = (h_pool2_flat.matmul(W_fc1) + b_fc1).relu()
        
        let y_conv = (h_fc1.matmul(W_fc2) + b_fc2).softmax()
        return (y_conv.elements,y_conv.elements.enumerated().max {$0.1 < $1.1}!.0)
    }
}

extension Classifier {
    public init(path: String) {
        W_conv1 = Tensor(shape: [5, 5, 3, 32], elements: loadFloatArray(path, file: "W_conv1"))
        ...
        W_fc1 = Tensor(shape: [(Dimension(7) * Dimension(7) * 64), 1024], elements: loadFloatArray(path, file: "W_fc1"))
        b_fc1 = Tensor(shape: [1024], elements: loadFloatArray(path, file: "b_fc1"))
        W_fc2 = Tensor(shape: [1024, 3], elements: loadFloatArray(path, file: "W_fc2"))
        b_fc2 = Tensor(shape: [3], elements: loadFloatArray(path, file: "b_fc2"))
    }
}
